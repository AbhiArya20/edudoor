...
class TrackingService {
  ...
  static void logScreenView(String screenName) {
    _analytics.logScreenView(screenName: screenName, parameters: {
      // "user_id": AllBloc.authenticationBloc.employerModel?.id ?? "UNKNOWN",
      // 'user_email': AllBloc.authenticationBloc.employerModel?.email ?? "UNKNOWN",
      // "user_phone": AllBloc.authenticationBloc.employerModel?.phone ?? "UNKNOWN",
      // "user_name": AllBloc.authenticationBloc.employerModel?.name ?? "UNKNOWN",
  ...
  static void logNavigation(String? from, String? to) {
    Log.trace(tag: "NAVIGATION", message: "Navigating from ${from ?? "unknown_page_OR_dialog"} to ${to ?? "unknown_page_OR_dialog"}");
